...
export default function PrixNegocie({ prixInitial, surface }: PrixNegocieProps) {
  ...
  const [isEditingLoyer, setIsEditingLoyer] = useState(false);
            ...
            {isEditingLoyer ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      setIsEditingLoyer(false);
                    } else if (e.key === 'Escape') {
                      setIsEditingLoyer(false);
                      setLoyerMensuel('');
                    }
                  }}
                  onBlur={() => setIsEditingLoyer(false)}
                ...
                <span>€/mois</span>
                <button
                  onClick={() => setIsEditingLoyer(false)}
                  className="text-sm bg-indigo-100 text-indigo-700 px-2 py-1 rounded hover:bg-indigo-200"
                >
                  OK
                </button>
              ...
              <div 
                ...
                onClick={() => {
                  ...
                  setIsEditingLoyer(true);
                }}
              >
                <span className="text-indigo-600 font-medium">
                  {loyerMensuel ? `${parseInt(loyerMensuel).toLocaleString('fr-FR')} €/mois` : 'Définir le loyer'}
                </span>
